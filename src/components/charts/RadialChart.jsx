...
import { WorkData } from '../sampleData';
import { Link } from 'react-router-dom';

const RadialChart = () => {
  const fabricator = WorkData[0]; // Assuming you want to show data for the first fabricator

  // Extract project statuses
  const projectStatuses = fabricator.project.reduce((acc, project) => {
    acc[project.status] = (acc[project.status] || 0) + 1;
    return acc;
  }, {});

  const statuses = Object.keys(projectStatuses);
  const counts = Object.values(projectStatuses);

  useEffect(() => {
    if (document.getElementById("radial-chart")) {
      const chart = new ApexCharts(document.querySelector("#radial-chart"), getChartOptions(statuses, counts));
      chart.render();
    }
  }, [statuses, counts]);
      ...
      <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
        ...
        >
          ...
          <dl className="flex items-center justify-between">
            ...
            <dd className="bg-red-100 text-red-800 text-xs font-medium inline-flex items-center px-2.5 py-1 rounded-md dark:bg-red-900 dark:text-red-300">
              <svg
                className="w-2.5 h-2.5 me-1.5"
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
...
const getChartOptions = (statuses, counts) => ({
  series: counts,
  labels: statuses,
  chart: {
    type: 'donut',
